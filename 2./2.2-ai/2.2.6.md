...
3. 워크플로우 화면이 표시되며, 좌측의 \[에이전트 생성] 버튼을 클릭하거나, 시작(주황색) 노드 하단의 \
   \[에이전트 가져오기] 버튼을 클릭합니다.&#x20;
...
1. 좌측의 **\[에이전트 생성]** 버튼을 클릭하고나서 에이전트 이름을 입력한 다음, 하단의\[**추가]** 버튼을 클릭합니다.
...
{% hint style="info" %}
**주의:** 자신에게 공유되지 않은 모델, 페르소나, 도구가 포함된 에이전트의 경우, 정상적으로 작동하지 않을 수 \
있습니다.
{% endhint %}
...
* 연결은 점 표시를 누른 상태에서 드래그하여 에이전트와 연결하고, 에이전트의 점 표시를 눌러 드래그하여 \
  답변 노드와 연결합니다.
...
{% hint style="info" %}
**주의:** 자신에게 공유되지 않은 모델, 페르소나, 도구가 포함된 에이전트가 있는 경우, 정상적으로 작동하지 않을 수 \
있습니다.
{% endhint %}
...
3. 프롬프트의 질문에 따른 응답이 출력됩니다. 또한 하단의 채팅 세부 내역을 통해 대화 통계와 토큰 사용량을 확인할 수 있습니다.

<div align="left"><figure><img src="../../.gitbook/assets/image (42).png" alt=""><figcaption></figcaption></figure></div>

4. 신규 채팅 진행을 원할 경우, 좌측 상단의 **\[대화 생성]** 버튼을 클릭하면 새 대화 화면에서질문 및 답변을 진행할 수 있습니다.
