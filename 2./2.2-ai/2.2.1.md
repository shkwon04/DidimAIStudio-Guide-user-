...
3. **새 모델 생성**: AI 모델을 로컬/클라우드로 분류하여 생성할 수 있습니다. 모델 정보, API 연동, 파라미터 조정, 권한 등을 설정할 수 있으며, 설정 완료 후 하단의 **\[AI 모델 생성하기]** 버튼을 클릭합니다.
4. **모델 목록**: 선택 옵션에 따라 모델 목록이 조회되며, 모델명과 함께 연결 상태, 평점, 모델의 활용 분야를 키워드로 확인할 수 있습니다.
...
1. 모델 페이지 상단의 \[전체] 탭에서 내 모델로 가져올 항목을 클릭합니다.
